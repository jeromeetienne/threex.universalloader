...
THREEx.UniversalLoader = function(manager) {
    // use a downlad manager, to track the progress
...
THREEx.UniversalLoader.prototype.load = function(urls, onLoad) {
    ...
    if (urls[0].match(/\.stl$/i) && urls.length === 1) {
        this.loader = new THREE.STLLoader(this.manager);
        ...
        // colladaLoader has no manager support ( bypass using itemStart and itemEnd )
        this.loader = new THREE.ColladaLoader();
        this.manager.itemStart(urls[0]);
        this.loader.options.convertUpAxis = true;
        this.loader.load(urls[0], function(collada) {
            // console.dir(arguments)
            var object3d = collada.scene
            _manager.itemEnd(urls[0]);
            onLoad(object3d);
        });
        return;
        ...
        this.manager.itemStart(urls[0]);
        this.manager.itemStart(urls[1]);
        this.loader.load(urls[1], urls[0], function(object3d) {
            _manager.itemEnd(urls[0]);
            _manager.itemEnd(urls[1]);
            onLoad(object3d);
        });
        return;
